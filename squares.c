...
void	find_lines(t_line** lines, t_square* square, uint16_t a, uint16_t b, uint16_t c, uint16_t d)
{
	...
	square->old_colors[0] = lines[a][b - a - 1].color;
	square->old_colors[1] = lines[a][c - a - 1].color;
	square->old_colors[2] = lines[a][d - a - 1].color;
	square->old_colors[3] = lines[b][c - b - 1].color;
	square->old_colors[4] = lines[b][d - b - 1].color;
	square->old_colors[5] = lines[c][d - c - 1].color;
}
